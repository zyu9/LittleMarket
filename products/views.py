...
def index(request):
  ...
  if (request.GET.get("add-cart")):
    ...
    if (CartItem.objects.filter(name=name).count()) == 0:
      cart_item = CartItem.objects.create(name=name, price=float(price), quantity=quantity, image=image)
      cart_item.save()
    else:
...
def new(request):
  if (request.GET.get("submit")):
    ...
    image = request.GET.get("image_input")

    product = Product.objects.create(name=name, price=float(price), stock=int(stock), image=image)
    product.save()
